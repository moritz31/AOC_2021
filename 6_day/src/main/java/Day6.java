...
//        String fishInput = "3,4,3,1,2";
        List<Long> fishes = Arrays.asList(0L, 0L, 0L, 0L, 0L, 0L, 0L, 0L, 0L);

        Arrays.stream(fishInput.split(",")).forEach(input -> fishes.set(Integer.parseInt(input), fishes.get(Integer.parseInt(input)) + 1));

        for(int i = 0; i <  256; i++) {
            Collections.rotate(fishes,  -1);
            fishes.set(6, fishes.get(6) + fishes.get(8));
        }
        System.out.println(fishes.stream().reduce(0L, Long::sum));
    }
}
